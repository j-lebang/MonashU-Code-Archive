...
class EffectivenessCalculator:
    ...
    def __init__(self, element_names: ArrayR[str], effectiveness_values: ArrayR[float]) -> None:
        ...
        """
        EffectivenessCalculator.n = len(element_names)
        EffectivenessCalculator.effectiveness = ArrayR(EffectivenessCalculator.n)

        for i in range(EffectivenessCalculator.n): # O(N)
            temp = ArrayR(EffectivenessCalculator.n)
            for j in range(EffectivenessCalculator.n): # O(N)
                temp[Element.from_string(element_names[j]).value - 1] = effectiveness_values[i * EffectivenessCalculator.n + j]
            EffectivenessCalculator.effectiveness[Element.from_string(element_names[i]).value - 1] = temp

        # Final Complexity:
        # Best Case: O(N^2)
        # Worst Case: O(N^2)
        # Since given the csv, complexity would be O(18^2) = O(1)
    ...
    def get_effectiveness(cls, type1: Element, type2: Element) -> float:
        ...
        """
        return cls.effectiveness[type1.value - 1][type2.value - 1]
